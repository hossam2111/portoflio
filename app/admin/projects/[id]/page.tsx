    ...
    async function loadData() {
      try {
        ...
        if (projectRes.ok) {
          ...
          setProcessField(project.process || "");
          setResults(project.results || "");
          setTechnicalSpecs(project.technical_specs || "");
          ...
          if (project.completion_date) {
            // format completion date as yyyy-MM-dd for HTML input
          ...
          if (Array.isArray(project.technologies)) {
            setTechnologies(project.technologies.map((t: any) => t.technology_name));
          }
          if (Array.isArray(project.materials)) {
            setMaterials(project.materials.map((m: any) => m.material_name));
          }
          if (Array.isArray(project.project_media)) {
            setGallery(
              project.project_media.map((pm: any) => ({
                file_url: pm.file_url,
                caption: pm.caption || undefined,
                media_type: pm.media_type || "image",
              }))
            );
          }
        } else {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#64748B]">Update project details and settings.</p>
          </div>
        </div>
